...
*/
var colors = ["red", "blue", "green", "brown", "cyan", "purple", "orange", "pink", "white", "lime", "magenta", "teal", "silver", "yellow", "gold", "yellowgreen"];
var sentences1 = ["Eine Variable die mit „const“ deklariert wurde…", "Mit Array- name.push wird ein Wert…", "Mit Array- name.pop() wird ein Wert…", "Um Typescript im Browser zu verwenden muss…", "HTML ist…", "CSS ist…", "TypeScript ist…", "Die IF-Anweisung…", "Eine for-schleife…", "i++ entspricht…", "Der EventListener…", "Im Hexadezimal- system…", "Die Aussage „Die erste Stelle eines Arrays hat immer den Index 0“ ist…", "Die Aussage „Es gibt auch fußgesteuerte Schleifen“ ist…", "Der Vergleichs- operator „>=“ bedeutet…", "Der Vergleichs- operator „!=“ bedeutet…"];
var sentences2 = ["…lässt sich nachträglich nicht mehr ändern.", "…am Ende des Arrays hinzugefügt.", "…am Ende des Arrays entfernt.", "…es erst zu Javascript transpiliert werden.", "…das Grundgerüst und der Inhalt einer Webseite.", "…für die Gestaltung und das Aussehen zuständig.", "…für Veränderung des Inhalts und des Aussehens zuständig.", "…erlaubt, Bedingungen für den Code aufstellen.", "…wird ausgeführt, bis die Bedingung nicht mehr erfüllt ist.", "… i = i + 1.", "…wird aktiviert, wenn ein definiertes Ereignis stattfindet.", "…sind 15 Werte pro Stelle/Zeichen möglich.", "…Falsch.", "…Wahr", "…größer gleich.", "…nicht gleich."];
var icons = ["assets/icons/address-book.svg", "assets/icons/building.svg", "assets/icons/calendar.svg", "assets/icons/clock.svg", "assets/icons/eye.svg", "assets/icons/flag.svg", "assets/icons/heart.svg", "assets/icons/laugh.svg"];
/*
...
var easycards = cards.slice(0, 8); // Karten für das leichte Spiel sind die ersten 8 (zu beachten, bei slice wird letztes Element nicht miteinbezogen, daher Ende 8 und nicht 7)
var mediumcards = cards.slice(0, 16); // Karten für das mittlere Spiel sind die ersten 16
var hardcards = cards; // Karten für das schwere Spiel sind alle
var difficulty; // Für die Zusammenfassung am Ende
var scoreplayer;
var scorenpc;
var totalscore;
var playerturn = true;
var flippedcards = 0; // Anzahl der aktuell gedrehten Karten
var flippedcolor = []; // Array, welches die vom Spieler aktuell umgedrehten Karten erhält
var counter; // Die Sekunden, wie lange das Spiel schon geht.
var timer; // Wird später der Timer zugewiesen, ist notwendig um den Timer stoppen zu können.
// Einige öfters verwendete HTMLElemente bekommen eigene Variablen zugewiesen.
var main = document.querySelector(".main"); // In Main werden die Karten platziert
var popup = document.querySelector(".popup"); // Das Popup ist für die Zusammenfassung und die Auswahl der Schwierigkeit zuständig
var resultmessage = document.getElementById("resultmessage"); // Hier wird am Ende die Zusammenfassung reingeschrieben
var win = new Audio("assets/sounds/win.mp3"); // Fanfare bei Sieg
var loss = new Audio("assets/sounds/loss.mp3"); // Traurige Trompete bei Niederlage
function runtimer() {
...
function setdifficulty() {
    ...
    document.querySelector("#leicht").addEventListener("click", function () {
        starteasygame();
    });
    document.querySelector("#mittel").addEventListener("click", function () {
        startmediumgame();
    });
    document.querySelector("#schwer").addEventListener("click", function () {
        starthardgame();
    });
}
...
function reset() {
    ...
    easycards.sort(function (a, b) { return 0.5 - Math.random(); }); // Die drei Arrays der verschiedenen Schwierigkeiten werden zufällig gemischt, hier recht einfach
    mediumcards.sort(function (a, b) { return 0.5 - Math.random(); });
    hardcards.sort(function (a, b) { return 0.5 - Math.random(); });
...
function randomplacement(cardamount) {
    for (var index = 0; index < cardamount; index++) { // For schleife läuft so oft durch, wie Karten aufs Feld gebracht werden sollen.
        var newElement = document.createElement("div"); // Für die Rückseite wird ein neues Element einer Variable zugewiesen
        var frontside = document.createElement("div"); // Für die Vorderseite ebenso
        switch (cardamount) {
            case 8:
                var coloreasy = " invis" + " " + easycards[index].color + " card";
                ...
                var easyPictogramm = document.createElement("img"); // im Falle der Karten mit Piktogramm wird ein neues Bildelement erstellt.
                ...
                break;
            case 16:
                var colormedium = " invis" + " " + mediumcards[index].color + " card";
                var mediumPictogramm = document.createElement("img");
                ...
                break;
            case 32:
                var colorhard = " invis" + " " + hardcards[index].color + " card";
...
function scoring(card1, card2) {
    ...
    if (flippedcards === 2) { // if ist hier eigentlich nicht notwendig, vorher war cardflip und scoring eine Funktion. never touch a running system
        if (card1.className === card2.className) { // Die Farbe wird verglichen, Farbe ist bei allen Schwierigkeiten ein Alleinstellungsmerkal der Paare
            ...
            setTimeout(function () {
                card1.parentElement.classList.toggle("found"); // Beide gefundenen Karten werden nach einem Delay unsichtbar und unklickbar
...
function npcturn() {
    ...
    // console.log(playerturn);
    var unfoundcards = document.querySelectorAll("div:not(.found).back");
    ...
    if (playerturn === false && unfoundcards.length > 1) { // Wenn der Computer dran sind und noch Karten auf dem Feld sind führt er seinen Zug durch
        ...
        var cardpicked1 = Math.round((Math.random() * (unfoundcards.length - 0.499)) - 0.5); // eine auf ganze Zahlen gerundete Zahl zwischen 0 und der Maximalen Anzahl der Karten auf dem Feld wird erstellt 
        do {
            var cardpicked2 = Math.round((Math.random() * (unfoundcards.length - 0.499)) - 0.5); // erneute Ziehung für die  Zweite Karte, solange bis sie nicht gleich der ersten ist.
        } while (cardpicked1 === cardpicked2);
        // console.log(cardpicked1 + " " + cardpicked2);
        var temppicked1 = unfoundcards[cardpicked1].children[0]; // Aus dem Array mit allen ungefundenen Karten wird die Karte mit dem Index der zufälligen Zahl in eine Variable kopiert.
        var temppicked2 = unfoundcards[cardpicked2].children[0];
...
*/
function game(totalpairs) {
    runtimer();
    totalscore = totalpairs; // Anzahl der Paare wird einer Variable zugewiesen, die auch außerhalb der Funktion verfügbar ist.
    var placedcards = document.querySelectorAll(".back"); // Es werden alle Karten ausgewählt und einer Variable zugewiesen
    placedcards.forEach(function (card) { return card.addEventListener("click", cardflip); }); // Bei allen Karten wird auf Klick gewartet, wenn geklickt wird die Funktion cardflip ausgeführt
}
/*
